...
- [x] Template AnimBP (2026-08-14): `/Game/ARKitRemap/abp_arkit_remap_live` — Live Link Pose (subject default `webcam`, editable on the node) → Rig Mapper (Definition = RM_MHA_to_ARKit, Alpha pin 1.0) → Output Pose. On the UE5 Mannequin skeleton (what the Fab cyclops rides; face is all morphs so skeleton only gates ABP compatibility). Built programmatically via Epic MCP BlueprintTools (`create_node` type_ids `LiveLink|LiveLinkPose` + `Animation|Poses|RigMapper`, `connect_pins`, `set_pin_value` for the subject literal; Definitions set via python struct write-back on the AnimGraphNode). **To test live: stream MHA webcam → set a cyclops's Anim Class to `abp_arkit_remap_live_C`.** Also done: `SK_cyclops_base_morph` stamped with `RigMapperDefinitionUserData` → [RM_MHA_to_ARKit] (the "tag your character once" path; anim node + retarget ops auto-discover it).
- [x] Instance-editable controls (2026-08-14): template ABP now has `UseLiveLink` (bool, default true) + `LiveLinkSubject` (LiveLinkSubjectName, default `webcam`) variables wired in-graph — subject var feeds the LiveLink Pose pin; `Blend Poses by Bool` (true=BlendPose_0=remap chain, false=LocalSpaceRefPose) gated by UseLiveLink. Gotcha: BlendListByBool active child = index 0. MetaHuman-style ACTOR Details checkboxes (like BP_MH_dyl2's Live Link section) = instance-editable vars on the character BP whose event graph pushes into the anim instance — documented pattern, per-character work.
- [x] Details-panel parity component (2026-08-14): `/Game/ARKitRemap/BC_ARKitRemapLive` (ActorComponent BP, built via BlueprintTools create + write_graph_dsl) — instance-editable UseLiveLink/LiveLinkSubject/UseHeadMovement; EventTick: GetOwner → GetComponentByClass(SkeletalMeshComponent) → GetAnimInstance → CastToabp_arkit_remap_live → push the three vars. Add the component to any char BP = MetaHuman-style checkboxes on the actor. DSL gotchas: component tick event id = `AddEvent|EventTick` (not ReceiveTick); class-var setters appear as `Class|AbpArkitRemapLive|SetX` node ids.
- [ ] `BPI_RigMapper` (EnableRigMapper / OverrideDefinitions) interface implementation on character BPs — superseded in practice by BC_ARKitRemapLive; keep as optional Epic-contract compliance.
- [x] First user guide draft (2026-08-14): `docs/USER-GUIDE.md` — plain-language concepts table, the three usage paths click-by-click, tagging explained, quality expectations, limitations/roadmap, FAQ. For the public release; grows with P4.
...
- [x] Head movement option (2026-08-14): `UseHeadMovement` toggle on the template ABP, **default OFF** (Dylan's spec). Event graph (authored via write_graph_dsl) reads HeadRoll/HeadPitch/HeadYaw curve values per tick → three scaled rotator vars; AnimGraph chains 3 Transform (Modify) Bone nodes (neck_01 25% / neck_02 30% / head 45%, additive, component space) between the blend and output — distributing rotation down the neck chain so shoulders/body follow naturally like the MetaHuman does. Space-conversion gotcha: skeletal-control nodes need explicit LocalToComponent/ComponentToLocal nodes when wiring via MCP (no auto-insert). Signs/axes unverified against a real take yet — flagged for Dylan's next live test. HeadTranslation not passed (rarely meaningful on non-MetaHuman rigs; revisit on demand). No ScaleRotator node exists in 5.8 — scale the floats before MakeRotator.
